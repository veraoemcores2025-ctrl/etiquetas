...
export async function createWbuyLabelPdf(orderResponse) {
  ...
  page.drawRectangle({ x: 0, y: pageHeight - 36, width: pageWidth, height: 36, color: black });
  page.drawText(carrier.slice(0, 26), { x: 10, y: pageHeight - 22, size: 12, font: bold, color: rgb(1, 1, 1) });
  page.drawText(`PEDIDO #${orderId}`, { x: pageWidth - 92, y: pageHeight - 22, size: 9, font: bold, color: rgb(1, 1, 1) });

  y = pageHeight - 54;
  page.drawText("DESTINATARIO", { x, y, size: 10, font: bold, color: gray });
  y -= 16;
  y = drawWrapped(page, recipientName, x, y, { font: bold, size: 15, lineHeight: 17, maxChars: 25, maxLines: 2 });
  y = drawWrapped(page, recipientLine1, x, y - 2, { font: bold, size: 11, lineHeight: 13, maxChars: 31, maxLines: 2 });
  y = drawWrapped(page, recipientLine2, x, y - 1, { font: bold, size: 11, lineHeight: 13, maxChars: 31, maxLines: 2 });

  const qrSize = 48;
  page.drawImage(qrImage, { x: pageWidth - qrSize - 10, y: pageHeight - 129, width: qrSize, height: qrSize });

  page.drawRectangle({ x, y: y - 22, width: 102, height: 19, color: black });
  page.drawText(`CEP: ${recipientCep}`, { x: x + 6, y: y - 16, size: 12, font: bold, color: rgb(1, 1, 1) });
  if (phone) page.drawText(`TEL: ${phone}`, { x: x + 112, y: y - 15, size: 8, font, color: black });
  ...
  y -= 16;
  page.drawRectangle({ x: 10, y: y - 28, width: pageWidth - 20, height: 40, borderColor: light, borderWidth: 1 });
  page.drawText("RESUMO DO PEDIDO", { x, y, size: 8, font: bold, color: gray });
  y -= 13;
  ...
  for (const line of productLines) {
    page.drawText(line.slice(0, 54), { x: x + 2, y, size: 7, font, color: black });
    y -= 9;
  }
  ...
  page.drawImage(barcodeImage, { x: 19, y: 34, width: barcodeWidth, height: barcodeHeight });
  page.drawText(`CODIGO: ${barcodeText}`, { x: 16, y: 22, size: 8, font: bold, color: black });
